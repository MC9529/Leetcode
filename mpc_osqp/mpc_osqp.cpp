...
MpcOsqp::MpcOsqp(const Eigen::MatrixXd &matrix_a,const Eigen::MatrixXd &matrix_b,const Eigen::MatrixXd &matrix_q,
    ...
    : matrix_a_(matrix_a), matrix_b_(matrix_b), matrix_q_(matrix_q),matrix_r_(matrix_r), matrix_initial_x_(matrix_initial_x),
    matrix_u_lower_(matrix_u_lower), matrix_u_upper_(matrix_u_upper), matrix_x_lower_(matrix_x_lower), matrix_x_upper_(matrix_x_upper), 
    matrix_x_ref_(matrix_x_ref), max_iteration_(max_iter), horizon_(horizon),eps_abs_(eps_abs) {
...
Eigen::MatrixXd csc2matrix (csc *cscA)
{
  ...
  for(int j =0; j<cscA->nzmax; j++)
  {
    int m = (cscA->i)[j];
    int n;
    for(int k = 0; k < (cscA->n) +1; k++ )
    {
      if ((cscA->p)[k] <= j && (cscA->p)[k+1] > j)
      {
        n = k;
      }
    }

    res_matrix[m][n] = (cscA->x)[j];
  }
  return res_matrix;
...
void MpcOsqp::CalculateKernel(std::vector<c_float> *P_data,
  ...
  for (size_t i = 0; i < state_dim_ * (horizon_ + 1); ++i) {
    for (const auto &row_data_pair : columns[i]) {
      std::cout << "the columns first:"<<row_data_pair.first << std::endl ;    // val
      std::cout << "the columns second :"<<row_data_pair.second << std::endl;                                                               // row
    }
  }
}
...
OSQPData *MpcOsqp::Data() {
  ...
  ADEBUG << "Get P matrix";
  std::cout << "the P matrix :\n"<< data->P << std::endl;

  data->q = gradient_.data();
  std::cout << "the q matrix :\n"<< data->q << std::endl;
...
bool MpcOsqp::Solve(std::vector<double> *control_cmd) {
  ...
  std::cout << "OSQP data n :" << data->n << std::endl;

  ADEBUG << "OSQP data m" << data->m;
  std::cout << "OSQP data m :" << data->m << std::endl;

  for (int i = 0; i < data->n; ++i) {
    ADEBUG << "OSQP data q" << i << ":" << (data->q)[i];
    std::cout << "OSQP data q :" << i << ":" << (data->q)[i] << std::endl;
  }
  ADEBUG << "OSQP data l" << data->l;
  for (int i = 0; i < data->m; ++i) {
    ADEBUG << "OSQP data l" << i << ":" << (data->l)[i];
    std::cout << "OSQP data l :" << i << ":" << (data->l)[i] << std::endl;
  }
  ADEBUG << "OSQP data u" << data->u;
  for (int i = 0; i < data->m; ++i) {
    ADEBUG << "OSQP data u" << i << ":" << (data->u)[i];
    std::cout << "OSQP data u :" << i << ":" << (data->u)[i] << std::endl;
  }
