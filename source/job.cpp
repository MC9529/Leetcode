...
Bitnode<int> *offer07::dfs(vector<int> &preorder, vector<int> &inorder,
...
bool offer12::exit(vector<vector<char>> &board, string word) {
...
void offer13::dfs(int &row, int &col, int m, int n, int k, int &temp,
...
void offer29::travel(vector<vector<bool>> &visited, vector<vector<int>> &matrix,
...
vector<vector<int>> offer34::pathSum(Bitnode<char> *root, int sum) {
  if (!root) return {};
...
void offer34::dfs(Bitnode<char> *root, int sum, vector<int> &temp_res) {
  ...
  int val = (root->val) - 48;
  cout << "the char and int: " << root->val << " " << val << endl;
...
vector<int> searchRange_solution::searchRange(vector<int> &nums, int target) {
  const int l =
      distance(nums.begin(), lower_bound(nums.begin(), nums.end(), target));
  const int u = distance(nums.begin(),
                         prev(upper_bound(nums.begin(), nums.end(), target)));
...
vector<int> offer40::getKthNumber(vector<int> &nums, int k) {
  ...
  if (nums.size() == 0 || k > nums.size()) {
    cout << "the len of nums is zero or k is bigger than the len of nums"
         << endl;
    return {};
  }
...
int offer47::maxValue(vector<vector<int>> &grid) {
  ...
  cout << "the max: " << dp[grid.size() - 1][grid[0].size() - 1] << endl;
...
// offer60 n个赛子的点数
// n枚赛子，点数范围为（n, 6n), 数组长度为5n+1,
// 因为每个赛子指出的概率为1/6,所有n个赛子 每次指出概率为（1/6）^n
// ,如果有k种组合， 概率不为 k*（1/6）^n
...
// // 二叉树的最近公共祖先 offer681;
// 迭代版本
Bitnode<char> *offer681::lowestcommonancester(Bitnode<char> *root, int p_val,
                                              int q_val) {
  while (root) {
...
// 递归版本
Bitnode<char> *offer681::lowestcommonancester2(Bitnode<char> *root, int p_val,
                                               int q_val) {
...
Bitnode<char> *offer681::travel(Bitnode<char> *root, int p_val, int q_val) {
  if (!root) {
    return NULL;
  }
  int root_val = root->val - '0';
...
Bitnode<char> *offer682::lowestcommonancester(Bitnode<char> *root, int val_p,
                                              int val_q) {
  Bitnode<char> *res = NULL;
  if (root == NULL) {
    return NULL;
...
string leetcode12::int2roman(int num) {
  string res;
  vector<string> tmp_1 = {"",  "I",  "II",  "III",  "IV",
                          "V", "VI", "VII", "VIII", "IX"};
  vector<string> tmp_10 = {"",  "X",  "XX",  "XXX",  "XL",
                           "L", "LX", "LXX", "LXXX", "XC"};
  vector<string> tmp_100 = {"",  "C",  "CC",  "CCC",  "CD",
                            "D", "DC", "DCC", "DCCC", "CM"};
...
int leetcode13::roman2int(string s) {
  int res = 0;
  map<char, int> luomab = {{'I', 1},   {'V', 5},   {'X', 10},  {'L', 50},
                           {'C', 100}, {'D', 500}, {'M', 1000}};
  for (int i = 0; i < s.size(); ++i) {
...
int leetcode16::threeSumClosest(vector<int> &nums, int target) {
  ...
  for (int first = 0; first < n; ++first) {
    ...
    int left = first + 1, right = n - 1;
    while (left < right) {
      cout << "the first and left and right: " << first << " " << left << " "
           << right << endl;
  ...
  while (head2_temp != nullptr) {
    ...
    if (head2_temp->val < insert_node->val &&
      ...
      while (head2_temp->val >= insert_node->next->val) {
        if (insert_node->next != nullptr) {
          insert_node = insert_node->next;
          cout << "the insert node insert in 2542: " << insert_node->val
               << endl;
        }
        if (insert_node->next == nullptr) {
          break;
...
void leetcode22::dfs(vector<string> &res, string path, int n, int lc, int rc) {
  if (rc > lc || lc > n || rc > n) {
    return;
  }
  if (lc == n && rc == n) {
...
bool leetcode36::isvalidsuduku(vector<vector<char>> &board) {
  ...
  int col[9][10] = {0};
  // box[j/3 + (i/3) * 3][num] 第（j/3 +(i/3)*3）BOX第num元素存在与否  0 不存在
  // 1 存在
  int box[9][10] = {0};
  //
  for (int i = 0; i < 9; i++) {
...
void leetcode39::dfs(vector<int> &nums, vector<int> &temp, int target, int sum,
                     int posi) {
  if (sum == target) {
    res.push_back(temp);
    return;
...
void leetcode39::dfs1(vector<int> &nums, vector<int> &temp, int target, int sum,
                      int posi) {
  if (sum == target) {
    res.push_back(temp);
    return;
...
// 不可以重复利用
void leetcode40::dfs1(vector<int> &nums, vector<int> &temp, int target, int sum,
                      int posi) {
  if (sum == target) {
    res.push_back(temp);
    return;
...
void leetcode46::dfs(vector<bool> &visited, vector<int> &temp,
                     vector<int> &nums, int &posi) {
...
void leetcode47::dfs(vector<bool> &visited, vector<int> &temp,
                     vector<int> &nums, int &posi) {
  if (temp.size() == nums.size()) {
    ans.push_back(temp);
    return;
...
bool leetcode55::dfs(int cur_level, int &most_step, vector<int> &temp_step,
                     vector<int> &nums) {
  if (cur_level == nums.size() - 2 &&
      cur_level + nums[cur_level] < nums.size() - 1) {
...
// leetcode56 合并区间
void leetcode56::merge(vector<vector<int>> &nums, vector<pair<int, int>> &ans) {
  ...
  for (int i = 1; i < nums.size(); ++i) {
    if (nums[i][0] > min_begin && nums[i][0] < max_end) {
      ...
      cout << "the min_begin and max_end: " << min_begin << " " << max_end
           << endl;
...
void leetcode62::dfs(vector<pair<int, int>> &path, int &row, int &col, int m,
                     int n) {
  if (row == m - 1 && col == n - 1) {
    ans.push_back(path);
    return;
...
void leetcode63::dfs(vector<pair<int, int>> &path, int &row, int &col, int m,
                     int n) {
  if (row == m - 1 && col == n - 1) {
    ans.push_back(path);
    return;
...
void leetcode90::dfs(vector<int> &nums, vector<int> &temp, int posi,
                     vector<bool> &used) {
  ans.push_back(temp);
...
// leetcode123
/*
由于我们最多可以完成两笔交易，因此在任意一天结束后，我们会处于下面状态之一：
1. 未进行任何交易
2. 只进行了一次买的操作
3.进行了一次买和一次卖操作，即完成了一次交易
4. 完成了一笔交易的情况下，进行了第二次买的操作
5. 完成了全部的两笔交易
6. buy1   sell1   buy1  sell2 表示这些状态
*/
int leetcode123::maxprofit(vector<int> &prices) {
  int n = prices.size();
  ...
  cout << "the high profit of at most 2 times trade: " << sell2 << endl;
  return sell2;
}
...
/*
由于任何边缘的o不会标记为X，
我们可以想到，所有不被包围的O,应该直接或者间接与边界的O相连，
我们可以利用这个来判断o是否在边界上
*/
...
void leetcode130::dfs(vector<vector<char>> &board, int x, int y) {
...
vector<vector<string>> leetcode131::partition(string s) {
  res.clear();
  path.clear();
  backtracking(s, 0);
  for (int i = 0; i < res.size(); ++i) {
    vector<string> temp_res = res[i];
    for (int j = 0; j < temp_res.size(); ++j) {
      cout << temp_res[j] << " ";
    }
    cout << endl;
...
bool leetcode131::ispalindrome(const string &s, int start, int end) {
...
void leetcode131::backtracking(const string &s, int startIdex) {
  if (startIdex >= s.size()) {
    res.push_back(path);
    return;
  }
  for (int i = startIdex; i < s.size(); ++i) {
    if (ispalindrome(s, startIdex, i)) {
      string str = s.substr(startIdex, i - startIdex + 1);
      path.push_back(str);
    } else {
      continue;
    }
    backtracking(s, i + 1);  // 寻找i+ 1 为起始位置的子串
...
vector<vector<string>> leetcode132::partition(string s) {
  res.clear();
  path.clear();
  backtracking(s, 0);
  int len = 1e9;
  int flag = 0;
  for (int i = 0; i < res.size(); ++i) {
    int tmp_len = res[i].size() - 1;
    if (tmp_len < len) {
      len = tmp_len;
      flag = i;
    }
  }
  vector<string> temp_res = res[flag];
  for (int i = 0; i < temp_res.size(); ++i) {
    cout << temp_res[i] << " ";
  }
  cout << endl;
  cout << "the min operation: " << len << endl;
  cout << endl;
  return res;
}
bool leetcode132::ispalindrome(const string &s, int start, int end) {
  for (int i = start, j = end; i < j; ++i, --j) {
    if (s[i] != s[j]) {
      return false;
    }
...
void leetcode132::backtracking(const string &s, int startIdex) {
  if (startIdex >= s.size()) {
    res.push_back(path);
    return;
  }
...
//
int leetcode137::singlenum(vector<int> &nums) {
  int seen_once = 0, seen_twice = 0;
  for (auto i : nums) {
    seen_once = ~seen_twice & (seen_once ^ i);
    seen_twice = ~seen_once & (seen_twice ^ i);
  }
  cout << "the once: " << seen_once << endl;
  return seen_once;
}

// leetcode1081

string leetcode1081::smallestSubsequence(string s) {
  vector<bool> visited(26, false);  // 判断字母是否已经出现在栈中
 ...
 * 贪心算法
 * **************************************************************************/
// 为了让更多的孩子吃到饼干，应该先满足胃口的孩子，而且应该让他吃尽可能小的饼干
int leetcode455::findContentChildren(vector<int> &g, vector<int> &s) {
  int ans = 0;
  sort(g.begin(), g.end());
  sort(s.begin(), s.end());
...
int leetcode455::findContentChildren2(vector<int> &g, vector<int> &s) {
  sort(g.begin(), g.end());
  sort(s.begin(), s.end());
  ...
  cout << "the ans in way2: " << child << endl;
  return child;
}

//  leetcode135

int leetcode135::candy(vector<int> &ratings) {
  int ans = 0;
  vector<int> left(ratings.size());
  vector<int> right(ratings.size());
  vector<int> vec_ans(ratings.size());
...
int leetcode435::eraseOverlapIntervals(vector<vector<int>> &nums) {
  int ans = 0;
  if (nums.empty()) {
  ...
  cout << endl;
  int cur_posi = nums[0][1];
  for (int i = 1; i < nums.size(); ++i) {
    if (nums[i][0] < cur_posi) {
      // 有重复部分
      ans++;
    } else {
      // 没有重复跳到该位置
...
bool leetcode605::canPlaceFlower(vector<int> &flowered, int n) {
  bool ans = false;
  int count = 0;
  int first_posi = 0;
  for (int i = 0; i < flowered.size(); ++i) {
    if ((flowered[i] == 0 && i == 0) ||
        (flowered[i] == 0 && i == flowered.size() - 1) ||
        (flowered[i] == 0 && flowered[i - 1] == 0 && flowered[i + 1] == 0)) {
      cout << "i: " << i << endl;
      count++;
      flowered[i] = 1;
    }
